...
public class Widget extends Node {
	...
	@Export("getModel")
	public Model getModel(SequenceDefinition animation, int var2, boolean secondary, PlayerAppearance playerAppearance) {
		invalidate = false;
		int type;
		int id;
		if (secondary) {
			type = this.modelType2;
			id = this.modelId2;
		} else {
			type = this.modelType;
			id = this.modelId;
		...
		if (type == 0) {
			...
			Model model = (Model) Widget_cachedModels.get(id + (type << 16));
			if (model == null) {

				ModelData modelData;

				if (type == 1) {
					modelData = ModelData.ModelData_get(TaskHandler.Widget_modelsArchive, id, 0);
					if (modelData == null) {
						invalidate = true;
						return null;
					}
					model = modelData.toModel(64, 768, -50, -10, -50);
				}

				if (type == 2) {
					modelData = PacketBufferNode.getNpcDefinition(id).getModelData();
					if (modelData == null) {
						invalidate = true;
						return null;
					}
				...
				if (type == 3) {
					...
					modelData = playerAppearance.getModelData();
					if (modelData == null) {
						invalidate = true;
						return null;
					}
				...
				if (type == 4) {
					ItemDefinition item = Occluder.ItemDefinition_get(id);
					modelData = item.getModelData(10);
					if (modelData == null) {
						invalidate = true;
						return null;
					}

					model = modelData.toModel(item.ambient + 64, item.contrast * 5 + 768, -50, -10, -50);

				}

				Widget_cachedModels.put(model, id + (type << 16));
			}

			if (animation != null) {
				model = animation.transformWidgetModel(model, var2);
			}
